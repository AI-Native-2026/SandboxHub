...
- 入口：**http://43.135.120.107:8081**（HTTP 单端口，nginx 统一代理前端与 `/api`、`/realms`）
- 测试账号（密码均为 `Passw0rd!`）：

| 账号 | 角色 | 说明 |
|---|---|---|
| `admin` | platform-admin | 平台管理员，可见治理与管理菜单 |
| `tenantadmin` | tenant-admin | 租户管理员，管理本租户成员与配额 |
| `dev1` | developer | 普通开发者，自助创建/操作沙箱 |
| `viewer` | viewer | 只读，无任何写入口 |

> 右上角支持 **主题（暗/亮）** 与 **中 / EN 语言** 单按钮切换；多租户通过顶部「租户」下拉切换。
